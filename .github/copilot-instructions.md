...
- Use the provided **Makefile** commands for all build and test tasks:
  ...
  - `make netdump` to start mitmdump to analyze the HTTP traffic
        - The dump file will be under following path: `/tmp/mitmproxy.dump`
        - Always check if you have `make netdump` running before running new session with the tool. You can stop previous run using `kill` command.
...
- **Unit Tests:** For each new resource or data source, write unit tests covering all operations and edge cases. Use the `jarcoal/httpmock` library (already in the project) to simulate HTTP API responses.
  ...
  - The JSON response files should be based on real responses. Use `make netdump` and a terraform file or acceptance test to dump real JSON responses. Use the whole JSON payload for each request/response from the dump file. Don't use partial content.
